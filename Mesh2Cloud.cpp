...
void compute_face_normal(MatrixXF& Nf, VectorXF& f_areas, const MatrixXF& V, const MatrixXi& F)
{
	size_t nf = F.cols();
	Nf.resize(3, nf);
	f_areas.resize(nf);
...
bool Mesh2Cloud::convert(string filename)
{
	int view_num = total_view_num_;
	...
	for (int i = 0; i < Num; ++i)
	{
		...
		Vector3F pt = view_center_[v] + Float(x)*dx_[v] + Float(y)*dy_[v];
		
		Point p0(pt[0], pt[1], pt[2]);
		Point p1(pt[0] - view_dir_[v][0], pt[1] - view_dir_[v][1], pt[2] - view_dir_[v][2]);
	...
	save_ply(filename_pc + "_pointcloud.ply");
	return true;
}


bool Mesh2Cloud::save_ply(string filename)
{
